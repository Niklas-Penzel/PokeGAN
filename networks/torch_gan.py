...
dataroot = os.path.join("..", "clean_pkm")
...
class Generator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.ReflectionPad2d(1),
            nn.Conv2d(ngf*8, ngf*4, 3, 1, padding=0, bias=False),
            #PrintLayer(),
            ...
            nn.Upsample(scale_factor=2, mode='nearest'),
            nn.ReflectionPad2d(1),
            nn.Conv2d( ngf * 4, ngf * 2, 3, 1, padding=0, bias=False),
            ...
            nn.Upsample(scale_factor=2, mode='nearest'),
            nn.ReflectionPad2d(1),
            nn.Conv2d( ngf * 2, ngf, 3, 1, padding=0, bias=False),
            #PrintLayer(),
            nn.BatchNorm2d(ngf),
            nn.ReLU(inplace=True),
            ...
            nn.Upsample(scale_factor=2, mode='nearest'),
            nn.ReflectionPad2d(1),
            nn.Conv2d( ngf, nc, 3, 1, padding=0, bias=False),
            #PrintLayer(),
            nn.Tanh()
...
def train_torch_gan():
    ...
    dataset = dset.ImageFolder(root=dataroot,
                            transform=transforms.Compose([
                                ...
                                transforms.RandomAffine(5, translate=(0.1, 0.1), scale=None, shear=2, resample=False, fillcolor=(255,255,255)),
                                transforms.ToTensor(),
    ...
    netG = GeneratorTransposed(ngpu).to(device)
    ...
    netD = DiscriminatorStandard(ngpu).to(device)
    ...
    progress = 0
    d = datetime.now().strftime("%H-%M-%S_%d-%m-%Y")
    progress_path = os.path.join("..", "progress_pics", d)
